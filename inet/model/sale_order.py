...
class SaleOrder(models.Model):
    ...
    contact_id_inet = fields.Char('Contacto')
    ...
    commercial = fields.Float(string="Comercial", compute="_compute_commercial")
    presale1 = fields.Float(string="Preventa 1", compute="_compute_presale1")
    presale2 = fields.Float(string="Preventa 2", compute="_compute_presale2")
    total_comission = fields.Float(string="Comisión total", compute="_total_commission")
    ...
    def _compute_presale1(self):
        if self.one_part:
            self.presale1 = (self.one_part * float(self.commision_pre_real))/100
        else:
            self.presale1 = 0.0

    @api.depends('one_part')
    def _compute_presale2(self):
        if self.one_part > 0:
            self.presale2 = (self.two_part * float(self.commision_pre_real))/100
        else:
            self.presale2 = float(self.commision_pre_real)

    @api.depends('commision_real','commision_pre_real')
    def _total_commission(self):
        if self.commision_real:
            self.total_comission = float(self.commision_real) + float(self.commision_pre_real)
        else:
            self.total_comission = 0.0

    @api.depends('commision_real')
    def _compute_commercial(self):
        if self.commision_real:
            self.commercial = self.commision_real
        else:
            self.commercial = 0.0
