...
class AgingReceivable(models.TransientModel):
    _name = 'aging.receivable'
...
class AgingReceivableXls(models.AbstractModel):
    _inherit = 'download.file.base.model'
    _name = 'aging.receivable.xls'

    def init(self, record_id):
        super(AgingReceivableXls, self).init(record_id)
    ...
    def get_content(self):
        ...
        reg_aging = self.env['aging.receivable'].browse(self.record_id)
        ...
        ws = wbk.add_sheet('Anticuamiento por Cobrar')
        ...
        ws.write_merge(s0, s0, 0, 1, 'ANTICUAMIENTO DE COBRO', style0)
        ...
        if obj_invoice:
            for invoices in obj_invoice:
                ...
                state = 'Por Cobrar' if invoices.state == 'draft' or invoices.state == 'open' else 'Pagado'
